...
function ReviewSlider() {
    ...
    useEffect(() => {
        ;(async () => {
            const {data} = await apiConnecter("GET", ratingsEndpoints.REVIEWS_DETAILS_API)
            if(data?.success) {
        ...
        <Swiper slidesPerView={4} spaceBetween={25} loop={true} freeMode={true} autoplay={{
        ...
        }} modules={[FreeMode, Pagination, Autoplay]} className='w-full' >
          {
            reviews.map((review, i) => {
                return (
                    <SwiperSlide key={i} >
                        <div className='flex flex-col gap-3 bg-richblue-800 p-3 text-[14px] text-richblack-25'>
